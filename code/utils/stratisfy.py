...
def stratisfy_df(df, new_col_name, n_folds=10, nr_clean_folds=0):
    ...
    classes = sorted(
        list(set([item for sublist in plabels for item in sublist])))

    stratified_data_ids, stratified_data = stratify(
        plabels, classes, [1/n_folds]*n_folds, pquals, ecgs_per_patient, nr_clean_folds)
...
def stratify(data, classes, ratios, qualities, ecgs_per_patient, nr_clean_folds=1):
    ...
    # list of subset_sizes in terms of ecgs
    ...
    stratified_data_ids = [set()
                           for _ in range(len(ratios))]  # initialize empty
    ...
    while size > 0:
        ...
        try:
            ...
            label = min({k: v for k, v in lengths.items()
                        if v > 0}, key=lengths.get)
        except ValueError:
        ...
        # this is a list of all patient ids with this label sort by size descending
        unique_samples = unique_samples[idxs_sorted]
        # these are the corresponding counts
        ...
        for current_id, current_count in zip(unique_samples, unique_counts):
            ...
            if(qualities[current_id] < 1):
                subset_sizes_for_label = subset_sizes_for_label[:len(
                    ratios)-nr_clean_folds]

            # Find argmax clj i.e. subset in greatest need of the current label
            largest_subsets = np.argwhere(
                subset_sizes_for_label == np.amax(subset_sizes_for_label)).flatten()
            ...
            for x in per_label_data.keys():
                per_label_data[x] = [
                    y for y in per_label_data[x] if y != current_id]
